...
{   
    ...
    int A[] = {-1,20,30,35,40};
    create(A,5);
    g = first->next->next; //pointer g points to 30 
    h = first->next->next->next->next; //pointer h points to 40 of first
    h->next = g;   //establishing a loop. Last element of first points to 3rd element
    if(isLoop(first) == 1)
        printf("Given linkedlist has a loop\n");
    else
        printf("Not a loop\n");    
